...
def generate_diagnostics(
    ...
    lines += [
        ...
        f"  ADD base terms:  {available_add}",
        f"  REMOVE:          {current_term_names}",
        "",
        "Layer 3 combinators (combine existing base terms):",
        f"  interaction(term_a, term_b) — product of two terms (e.g., routine × recency)",
        f"  split_by(term, condition)   — term active only when condition > 0",
        f"  log_transform(term)         — signed log: sign(x) * log(1+|x|)",
        f"  threshold(term, cutoff=N)   — binary: 1 if term > N, else 0",
        f"  decay(term)                 — exponential time decay of term",
        f"  ratio(term_a, term_b)       — a / (b + eps), e.g., routine relative to popularity",
        f"  power(term, exponent=N)     — |x|^N * sign(x), e.g., diminishing returns on popularity",
        f"  difference(term_a, term_b)  — a - b, explicit contrast between two features",
        "",
...
def prompt_llm(
    ...
    if iteration < 2:
        ...
        anneal_note = "\nAll DSL operations including all combinators (interaction, split_by, threshold, log_transform, decay) are allowed. Propose up to 2 changes."
...
def apply_proposal(structure: DSLStructure, proposal: dict) -> Optional[DSLStructure]:
    ...
    for change in proposal.get("changes", []):
        ...
        if op == "ADD":
            ...
            # Try exact match by display_name, then by base name
            new_structure = new_structure.remove_term(raw_term)
            if len(new_structure) == len(structure):
                # remove_term didn't find it — nothing was removed
...
def _export_search_history(data_dir, history, proposal_log):
    ...
    # Build nodes from history (accepted path) + proposal_log (all proposals)
    accepted_structures = {}  # iteration -> structure string
    for h in history:
        node_id = f"accepted_{h['iteration']}"
        accepted_structures[h['iteration']] = h['structure']
        nodes.append({
            "id": node_id,
            ...
            "n_terms": len(h['structure'].replace('S = ', '').split(' + ')),
            "accepted": True,
            "type": "accepted",
        })
        # Edge from previous accepted to this accepted
        if h['iteration'] > 0:
            prev_id = f"accepted_{h['iteration'] - 1}"
            edges.append({"source": prev_id, "target": node_id, "type": "accepted"})

    # Add rejected proposals as separate nodes
    for p in proposal_log:
        if p.get("accepted"):
            continue  # already in the accepted path
        ...
        # Edge from parent accepted node to this rejected node
        parent_id = f"accepted_{p['iteration'] - 1}"
        edges.append({"source": parent_id, "target": node_id, "type": "rejected"})
    ...
    search_data = {
        ...
        "worst_score": min(n["score"] for n in nodes if n["score"] != 0),
    }
